...
def train_classical_models(X_train, y_train, seed: int):
    ...
    mlp = MLPRegressor(
        ...
        max_iter=200,
        random_state=seed,           # <-- use CLI seed for reproducibility
        early_stopping=True,
        n_iter_no_change=15,
        verbose=False,
    )
...
def train_optional_cnn(train_df, data_dir, img_size, out_dir, seed: int):
    ...
    X_train_i, X_val_i, y_train_i, y_val_i, m_train, m_val = train_test_split(
        X_imgs, y, male_feat, test_size=0.15, random_state=seed
    )
...
def main():
    ...
    scaler, svm, mlp = train_classical_models(X_tr, y_tr, seed=args.seed)
    ...
    results = {
        ...
        "val_acc_±12m_svm": tolerance_accuracy_months(y_va, p_svm_va, 12),
        "val_acc_±24m_svm": tolerance_accuracy_months(y_va, p_svm_va, 24),

        "val_acc_±12m_mlp": tolerance_accuracy_months(y_va, p_mlp_va, 12),
        "val_acc_±24m_mlp": tolerance_accuracy_months(y_va, p_mlp_va, 24),
    ...
    if args.use_cnn:
        model_path, cnn_mae = train_optional_cnn(
            df, args.data_dir, args.img_size, args.out_dir, seed=args.seed
        )
